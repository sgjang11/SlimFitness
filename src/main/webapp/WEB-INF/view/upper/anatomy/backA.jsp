<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<main>
	<div class="sub subList2">
		<ul>
			<li class="on"><a href="/upper/anatomy/backA">Anatomy</a></li>
			<li><a href="/upper/stretching/backS">Stretching</a></li>
			<li><a href="/upper/exercise/backE">Exercise</a></li>
			<li><a href="/upper/taping/backT">Taping</a></li>
		</ul>
	</div>
	<div class="sub subList3">
		<ul>
			<li class="on"><a href="/upper/anatomy/backA">등</a></li>
			<li><a href="/upper/anatomy/chestA">가슴</a></li>
			<li><a href="/upper/anatomy/shoulderA">어깨</a></li>
			<li><a href="/upper/anatomy/abdomenA">복부</a></li>
			<li><a href="/upper/anatomy/armA">팔</a></li>
		</ul>
	</div>
	<div class="searching">
		<input type="text" name="searching" id="searchBar">
		<button class="searchBtn"><i class="fa-solid fa-magnifying-glass"></i></button>
	</div>
	<div class="selectedList">
		<h3>이용방법</h3>
		<div>
			<img alt="${title}" src="/static/file/${filename1}">
			<img alt="${title}" src="/static/file/${filename2}">
			<img alt="${title}" src="/static/file/${filename3}">
			<img alt="${title}" src="/static/file/${filename4}">
		</div>	
				
	
	
		<input type="button" value="등록" onclick="location.href='/upper/manager/uploadForm'">
	</div>
</main>
